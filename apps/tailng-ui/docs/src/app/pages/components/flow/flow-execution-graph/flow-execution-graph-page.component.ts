...
type FlowExecutionGraphDocSectionId = 'api' | 'examples' | 'overview' | 'styling';

const flowExecutionGraphDocSectionIds: readonly FlowExecutionGraphDocSectionId[] = [
  'overview',
  'api',
  'styling',
  'examples',
...
function isFlowExecutionGraphDocSectionId(value: string): value is FlowExecutionGraphDocSectionId {
  return flowExecutionGraphDocSectionIds.includes(value as FlowExecutionGraphDocSectionId);
}

@Component({
  selector: 'app-flow-execution-graph-page',
  imports: [RouterOutlet, DocsComponentSectionTabsComponent, DocsComponentSectionOutlineComponent],
  templateUrl: './flow-execution-graph-page.component.html',
})
export class FlowExecutionGraphPageComponent {
  ...
  public readonly activeSection = computed<FlowExecutionGraphDocSectionId>(() => {
    const segments = this.normalizeUrl(this.currentUrl())
      .split('/')
      .filter((segment) => segment.length > 0);
    const section = segments[3];
    return section !== undefined && isFlowExecutionGraphDocSectionId(section)
      ? section
      : 'overview';
  });
  ...
  public readonly outlineAriaLabel = computed(() =>
    getDocsComponentSectionOutlineAriaLabel(
      this.docsItem?.title ?? 'Flow Execution Graph',
      this.activeSection(),
    ),
  );
