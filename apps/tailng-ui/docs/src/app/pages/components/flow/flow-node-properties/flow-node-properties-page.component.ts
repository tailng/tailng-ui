...
type FlowNodePropertiesDocSectionId = 'api' | 'examples' | 'overview' | 'styling';

const flowNodePropertiesDocSectionIds: readonly FlowNodePropertiesDocSectionId[] = [
  'overview',
  'api',
  'styling',
  'examples',
...
function isFlowNodePropertiesDocSectionId(value: string): value is FlowNodePropertiesDocSectionId {
  return flowNodePropertiesDocSectionIds.includes(value as FlowNodePropertiesDocSectionId);
}

@Component({
  selector: 'app-flow-node-properties-page',
  imports: [RouterOutlet, DocsComponentSectionTabsComponent, DocsComponentSectionOutlineComponent],
  templateUrl: './flow-node-properties-page.component.html',
})
export class FlowNodePropertiesPageComponent {
  ...
  public readonly activeSection = computed<FlowNodePropertiesDocSectionId>(() => {
    const segments = this.normalizeUrl(this.currentUrl())
      .split('/')
      .filter((segment) => segment.length > 0);
    const section = segments[3];
    return section !== undefined && isFlowNodePropertiesDocSectionId(section)
      ? section
      : 'overview';
  });
  ...
  public readonly outlineAriaLabel = computed(() =>
    getDocsComponentSectionOutlineAriaLabel(
      this.docsItem?.title ?? 'Flow Node Properties',
      this.activeSection(),
    ),
  );
