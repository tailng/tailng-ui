...
} from '../overview/flow-workbench-overview-code.data';

type FlowWorkbenchExamplesVariant = 'plain-css' | 'tailwind-css';

const examplesPaletteItems: readonly TngFlowPaletteItem<FlowEditorDemoData>[] = [
  {
...
const examplesModes: readonly { value: TngFlowWorkbenchMode; label: string }[] = [
...
const examplesExecutionSnapshot = {
  id: 'docs-agent-run',
...
@Component({
  selector: 'app-flow-workbench-examples-page',
  imports: [
    TngButtonComponent,
    TngFlowWorkbenchComponent,
    DocsExampleTabsSectionComponent,
  ...
  templateUrl: './flow-workbench-examples-page.component.html',
  styleUrl: './flow-workbench-examples-page.component.css',
})
export class FlowWorkbenchExamplesPageComponent implements OnDestroy {
  ...
  protected readonly executionSnapshot = examplesExecutionSnapshot;
  protected readonly modes = examplesModes;
  protected readonly paletteItems = examplesPaletteItems;
  ...
  protected onNodeCreateRequested(
    ...
    variant: FlowWorkbenchExamplesVariant,
  ): void {
  ...
  protected resetDemo(variant: FlowWorkbenchExamplesVariant): void {
  ...
  protected togglePalette(variant: FlowWorkbenchExamplesVariant): void {
    this.paletteOpenFor(variant).update((open) => !open);
  }

  protected toggleDetails(variant: FlowWorkbenchExamplesVariant): void {
  ...
  private selectionFor(variant: FlowWorkbenchExamplesVariant): WritableSignal<TngFlowSelection> {
    return variant === 'plain-css' ? this.plainCssSelection : this.tailwindSelection;
  }

  private inspectedNodeIdFor(variant: FlowWorkbenchExamplesVariant): WritableSignal<string | null> {
    return variant === 'plain-css' ? this.plainCssInspectedNodeId : this.tailwindInspectedNodeId;
  }

  private modeFor(variant: FlowWorkbenchExamplesVariant): WritableSignal<TngFlowWorkbenchMode> {
    return variant === 'plain-css' ? this.plainCssMode : this.tailwindMode;
  }

  private paletteOpenFor(variant: FlowWorkbenchExamplesVariant): WritableSignal<boolean> {
    return variant === 'plain-css' ? this.plainCssPaletteOpen : this.tailwindPaletteOpen;
  }

  private detailsOpenFor(variant: FlowWorkbenchExamplesVariant): WritableSignal<boolean> {
