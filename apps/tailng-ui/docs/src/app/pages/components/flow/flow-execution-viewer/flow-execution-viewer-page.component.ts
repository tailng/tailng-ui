...
type FlowExecutionViewerDocSectionId = 'api' | 'examples' | 'overview' | 'styling';

const flowExecutionViewerDocSectionIds: readonly FlowExecutionViewerDocSectionId[] = [
  'overview',
  'api',
  'styling',
  'examples',
...
function isFlowExecutionViewerDocSectionId(
  value: string,
): value is FlowExecutionViewerDocSectionId {
  return flowExecutionViewerDocSectionIds.includes(value as FlowExecutionViewerDocSectionId);
}

@Component({
  selector: 'app-flow-execution-viewer-page',
  imports: [RouterOutlet, DocsComponentSectionTabsComponent, DocsComponentSectionOutlineComponent],
  templateUrl: './flow-execution-viewer-page.component.html',
})
export class FlowExecutionViewerPageComponent {
  ...
  public readonly activeSection = computed<FlowExecutionViewerDocSectionId>(() => {
    const segments = this.normalizeUrl(this.currentUrl())
      .split('/')
      .filter((segment) => segment.length > 0);
    const section = segments[3];
    return section !== undefined && isFlowExecutionViewerDocSectionId(section)
      ? section
      : 'overview';
  });
  ...
  public readonly outlineAriaLabel = computed(() =>
    getDocsComponentSectionOutlineAriaLabel(
      this.docsItem?.title ?? 'Flow Execution Viewer',
      this.activeSection(),
    ),
  );
