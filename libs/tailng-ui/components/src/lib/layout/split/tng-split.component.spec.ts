...
async function createFixture() {
  ...
  const group = fixture.debugElement.children[0]
    .componentInstance as TngSplitGroupComponent;
...
describe('TngSplitGroupComponent', () => {
  it('defers ResizeObserver-driven layout work to an animation frame', async () => {
    ...
    const observerCallbackRef: {
      current: ResizeObserverCallback | null;
    } = {
      current: null,
    };
  ...
  it('resizes with keyboard steps and emits a final lifecycle event', async () => {
    ...
    expect(
      host.querySelectorAll<HTMLElement>('[tngSplitPane]')[0]?.style.width,
    ).toBe('282px');

    expect(resizeEnd).toHaveBeenCalledWith(
      expect.objectContaining({
        previousPaneId: 'palette',
        source: 'keyboard',
      }),
    );
  });
  ...
  it('switches to horizontal separators for vertical groups', async () => {
    ...
    expect(
      host.querySelector('tng-split-group')?.getAttribute('data-orientation'),
    ).toBe('vertical');

    expect(
      host.querySelector('tng-split-handle')?.getAttribute('aria-orientation'),
    ).toBe('horizontal');
  });
});